...
describe("/register", () => {
  describe("POST /", () => {
    ...
    const registerUsername = "testuser17";
      ...
      app
        ...
        .end((err, res) => {
          ...
          expect(res.text).to.equal(
            `Successfully logged in as: ${testUsername}`
          );
          if (err) return done(err);
          else return done();
        });
...
describe("GET /summary", function () {
  ...
  it('returns 200 + "a full summary object from 06-04-2022 to today"', (done) => {
    app.get("/summary/06-04-2022").end((err, res) => {
      console.log(`res code: ${res.statusCode} text: ${res.text}`);
      expect(res.statusCode).to.equal(200);
      fullSummaryObjectTest(res);
      if (err) return done(err);
  ...
  it("returns 200 + a full summary object for each entry", (done) => {
    app.get("/summary/06-04-1971").end((err, res) => {
      console.log(`res code: ${res.statusCode} text: ${res.text}`);
      expect(res.statusCode).to.equal(200);
      fullSummaryObjectTest(res);
      if (err) return done(err);
  ...
  it("returns 200 + a full summary object for 06-04-2022 to 06-09-2022", (done) => {
    app.get("/summary/06-04-2022/06-09-2022").end((err, res) => {
      console.log(`res code: ${res.statusCode} text: ${res.text}`);
      expect(res.statusCode).to.equal(200);
      fullSummaryObjectTest(res);
      if (err) return done(err);
  ...
  it('Returns 400 + "The entered date is invalid" when an invalid date is entered', (done) => {
    app.get("/summary/06-0d-2022").end((err, res) => {
      console.log(`res code: ${res.statusCode} text: ${res.text}`);
      expect(res.statusCode).to.equal(400);
  ...
  it('Returns 400 + "The entered date is in the future" when an entered date is in the future', (done) => {
    app.get("/summary/12-31-9999").end((err, res) => {
      console.log(`res code: ${res.statusCode} text: ${res.text}`);
      expect(res.statusCode).to.equal(400);
  ...
  it('Returns 400 + "The entered date is invalid" when an invalid date is entered', (done) => {
    app.get("/summary/06-02-2022/06-0d-2022").end((err, res) => {
      console.log(`res code: ${res.statusCode} text: ${res.text}`);
      expect(res.statusCode).to.equal(400);
      expect(res.text).to.equal("The entered date is invalid");
      if (err) return done(err);
...
describe("GET /journal-entries", function () {
  ...
  it("returns 200 + a full journal-entries array", (done) => {
    app.get("/journal-entries").end((err, res) => {
      console.log(`res code: ${res.statusCode} text: ${res.text}`);
      expect(res.statusCode).to.equal(200);
  ...
  it('returns 200 + the journal entry for 06-03-2022"', (done) => {
    app.get("/journal-entries/06-03-2022").end((err, res) => {
      console.log(`res code: ${res.statusCode} text: ${res.text}`);
      expect(res.statusCode).to.equal(200);
      fullJournalEntriesTest(res);
      if (err) return done(err);
  ...
  it("returns 200 + a full journal-entries array for dates 06-04-2022 to 06-09-2022", (done) => {
    app.get("/journal-entries/06-04-2022/06-09-2022").end((err, res) => {
      console.log(`res code: ${res.statusCode} text: ${res.text}`);
      expect(res.statusCode).to.equal(200);
      fullJournalEntriesTest(res);
      if (err) return done(err);
  ...
  it('Returns "The entered date is invalid" when an invalid date is entered', (done) => {
    app.get("/journal-entries/06-0d-2022").end((err, res) => {
      console.log(`res code: ${res.statusCode} text: ${res.text}`);
      expect(res.statusCode).to.equal(400);
      expect(res.text).to.equal("The entered date is invalid");
      if (err) return done(err);
  ...
  it('Returns "The entered date is in the future" when an entered date is in the future', (done) => {
    app.get("/journal-entries/12-31-9999").end((err, res) => {
      console.log(`res code: ${res.statusCode} text: ${res.text}`);
      expect(res.statusCode).to.equal(400);
      expect(res.text).to.equal("The entered date is in the future");
      if (err) return done(err);
  ...
  it('Returns "The entered date is invalid" when an invalid date is entered', (done) => {
    app.get("/journal-entries/06-02-2022/06-0d-2022").end((err, res) => {
      console.log(`res code: ${res.statusCode} text: ${res.text}`);
      expect(res.statusCode).to.equal(400);
      expect(res.text).to.equal("The entered date is invalid");
      if (err) return done(err);
...
describe("POST new entry, PUT (modify) it, then DELETE it", function () {
  ...
  describe("PUT /journal-entries", function () {
    it(`PUT JournalEntry that was just added, returns 200 + returns "Updated journal entry for user: ${testUsername}, at date: ${new Date(
      testDate
    )}."`, (done) => {
      app
        ...
        .end((err, res) => {
          ...
          expect(res.statusCode).to.equal(200);
          expect(res.text).to.equal(
            `Updated journal entry for user: ${testUsername}, at date: ${new Date(
              testDate
            )}.`
          );
          if (err) return done(err);
  ...
  describe("GET /journal-entries", function () {
    it(`GET JournalEntry for the date that was just added - returns 200 + the journal entry for ${testDate}"`, (done) => {
      app.get(`/journal-entries/${testDate}`).end((err, res) => {
        console.log(`res code: ${res.statusCode} text: ${res.text}`);
        expect(res.statusCode).to.equal(200);
        fullJournalEntriesTest(res);
        if (err) return done(err);
  ...
  describe("DELETE /journal-entries", function () {
    it(`DELETE JournalEntry that was just added, returns 200 + returns "Deleted journal entry for user: ${testUsername}, at date: ${new Date(
      testDate
    )}."`, (done) => {
      app.delete(`/journal-entries/${testDate}`).end((err, res) => {
        console.log(`res code: ${res.statusCode} text: ${res.text}`);
        expect(res.statusCode).to.equal(200);
        expect(res.text).to.equal(
          `Deleted journal entry for user: ${testUsername}, at date: ${new Date(
            testDate
          )}.`
        );
        if (err) return done(err);
...
describe("POST /journal-entries sad paths", function () {
  it(`Returns 400 and "Missing data in body: "weight"`, (done) => {
    ...
    app
      ...
      .end((err, res) => {
        console.log(`res code: ${res.statusCode} text: ${res.text}`);
        expect(res.statusCode).to.equal(400);
        expect(res.text).to.equal(`Missing data in body: "weight"`);
        if (err) return done(err);
        done();
      });
  });
  it(`Returns 400 and "Missing data in body: "exercise_goal_met"`, (done) => {
    let newEntryDataCopy = Object.assign({}, newEntryData);
    ...
    app
      ...
      .end((err, res) => {
        console.log(`res code: ${res.statusCode} text: ${res.text}`);
        expect(res.statusCode).to.equal(400);
        expect(res.text).to.equal(`Missing data in body: "exercise_goal_met"`);
        if (err) return done(err);
        done();
      });
  });
  it(`Returns 400 and "Missing data in body: "caloric_intake"`, (done) => {
    let newEntryDataCopy = Object.assign({}, newEntryData);
    ...
    app
      ...
      .end((err, res) => {
        console.log(`res code: ${res.statusCode} text: ${res.text}`);
        expect(res.statusCode).to.equal(400);
        expect(res.text).to.equal(`Missing data in body: "caloric_intake"`);
        if (err) return done(err);
        done();
      });
  });
  it(`Returns 400 and "Missing data in body: "protein_intake"`, (done) => {
    let newEntryDataCopy = Object.assign({}, newEntryData);
    ...
    app
      ...
      .end((err, res) => {
        console.log(`res code: ${res.statusCode} text: ${res.text}`);
        expect(res.statusCode).to.equal(400);
        expect(res.text).to.equal(`Missing data in body: "protein_intake"`);
        if (err) return done(err);
        done();
      });
  });
  ...
  it('Returns "The entered date is in the future" when an entered date is in the future', (done) => {
    app.post("/journal-entries/12-31-9999").end((err, res) => {
      console.log(`res code: ${res.statusCode} text: ${res.text}`);
      expect(res.statusCode).to.equal(400);
      expect(res.text).to.equal("The entered date is in the future");
      if (err) return done(err);
...
describe("PUT /journal-entries sad paths", function () {
  ...
  it('Returns "The entered date is in the future" when an entered date is in the future', (done) => {
    app.put("/journal-entries/12-31-9999").end((err, res) => {
      console.log(`res code: ${res.statusCode} text: ${res.text}`);
      expect(res.statusCode).to.equal(400);
      expect(res.text).to.equal("The entered date is in the future");
      if (err) return done(err);
