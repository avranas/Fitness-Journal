...
const weightChangeSummary = (journal) => {
  ...
  const avgDailyWeightChange =
    (startWeight - prevWeight) / (totalDayCounter - 1);
  return {
...
const goalSummary = (journal) => {
  ...
  const goalAchievedPercentage =
    ((goalAchievedCounter / journal.length) * 100).toString() + "%";
  return {
    maxChain,
...
const getWholeSummary = async (req, res, next, startDate, endDate) => {
  try {
    ...
    //Process data
    const { avgCaloricIntake, maxCaloricIntake, minCaloricIntake } =
      calorieSummary(journalResult);
    const { avgProteinIntake, maxProteinIntake, minProteinIntake } =
      proteinSummary(journalResult);
    const { avgDailyWeightChange, biggestGain, biggestLoss } =
      weightChangeSummary(journalResult);
...
summaryRouter.get("/", async (req, res, next) => {
  try {
    const summary = await getWholeSummary(
      req,
      res,
      next,
      new Date(0),
