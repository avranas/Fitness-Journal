...
journalEntryRouter.post("/:entry_date", async (req, res, next) => {
  try {
    ...
    if (req.body.weight === undefined) {
      throw createHttpError(400, 'Missing data in body: "weight"');
    }
    if (req.body.exercise_goal_met === undefined) {
      throw createHttpError(400, 'Missing data in body: "exercise_goal_met"');
    }
    if (req.body.caloric_intake === undefined) {
      throw createHttpError(400, 'Missing data in body: "caloric_intake"');
    }
    if (req.body.protein_intake === undefined) {
      throw createHttpError(400, 'Missing data in body: "protein_intake"');
    }
  ...
  try {
    ...
    if (!(await entryExists(req.user.id, entryDate))) {
      ...
      res
        .status(200)
        .send(
          `Updated journal entry for user: ${req.user.username}, at date: ${entryDate}.`
        );
    }
  } catch (err) {
    next(err);
  ...
  try {
    ...
    if (!(await entryExists(req.user.id, entryDate))) {
      ...
      res
        .status(200)
        .send(
          `Deleted journal entry for user: ${req.user.username}, at date: ${entryDate}.`
        );
    }
  } catch (err) {
    next(err);
